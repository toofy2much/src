...
def make_chains(rawtext):
    ...
    for i in range(len(words) - 1): 
        ...
        chain[key_tuple] = (words[i +1], words[i +2])
        #for loop where you take second word and then add every following word using some list slicing?
        #words[i + 1] - words [i]# gets overwritten each time thru loop, 
        # how to use prexisting value instead replace?
